...
def rename(source):
	...
	epoch = int(time.time())
	for input_file in os.listdir(source):
		process = subprocess.Popen('mv {} {}zeotap_{}_{}.tsv'.format(source+input_file, source, today, epoch), stdout=subprocess.PIPE, shell=True)
		output, error = process.communicate()
		epoch = epoch + 1
	print "Finished rename ..."


def start_upload_smartclip(source, destination, mode):
	global profile
	if mode == "PROD":
		profile = "Smartclip"
